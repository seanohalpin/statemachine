...
describe "Turnsule nested parallel" do
...
#    @sm.states.should == [:locked,:on]
  ...
  #  @sm.go
  #  @sm.process_event(:coin)
    # @sm.is_in_state?(:onoff).should == true TODO tests for superstates not working yet
    # @sm.is_in_state?(:operative).should == true
    #@sm.is_in_state?(:on).should == true

    # @sm.is_in_state?(:second).should == true ;; is not supported!!

#    @sm.process_event(:maintain) TODO not working yet!!!
